...
lsblk -o MOUNTPOINTS | sort |
{
	while read line; do
		if [[ $line =~ ^/.*$ ]]; then
			...
			out=$(df -h -BGiB "$line" | awk 'NR==2{print $6 ": " $3 " used out of " $2 " (" $5 ")"}')
			echo "$out"
	
			used=$(echo $out | awk '{print $2}' | numfmt --from=iec-i --suffix=B | tr -d -c 0-9)
	    total=$(echo $out | awk '{print $6}' | numfmt --from=iec-i --suffix=B | tr -d -c 0-9)
